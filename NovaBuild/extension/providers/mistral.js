...
// providers/mistral.js - Mistral (chat.mistral.ai) provider for Nova OP-Scripter.
...
const NVProvider = (() => {
  ...
  const S = {
    chatItem: "[data-testid*='message'], [class*='message']",
    markdown: ".prose, .markdown, [class*='markdown'], [class*='message-content']",
    editorSelectors: [
    ...
    sendBtn: 'button[type="submit"], button[aria-label*="Send"], button[data-testid*="send"]',
    stopBtn: 'button[aria-label*="Stop"]',
    errorSurfaces: '[role="alert"], [class*="toast"]',
  };

  const RE = {
    contextLimit: /context|too long|limit/i,
    tooLong: /too long/i,
    busy: /busy|try again|rate/i,
  ...
  function allItems() {
    return [...document.querySelectorAll(S.chatItem)].filter((el) => el.querySelector(S.markdown) || (el.textContent || "").trim().length > 20);
  }
  function isUserItem(item) {
    return item && (/user/i.test(item.getAttribute("data-testid") || "") || item.classList.contains("user") || !!item.querySelector("[data-testid*='user']"));
  }
  ...
  function snapshot() { return { provider: "mistral", assistants: assistantCount(), users: userCount() }; }
  ...
  function barMount() {
    const ed = getEditor();
    if (!ed) return null;
    const box = ed.closest("form") || ed.parentElement;
    if (!box) return null;
  ...
  async function ensureComposerReady(reason) {
    ...
    diag("mode_ready", { reason, provider: "mistral", ready });
    return { ready };
  }
  ...
  function openNewChat() {
    for (const b of document.querySelectorAll("a, button")) {
      if (RE.newChat.test((b.textContent || "").trim())) { b.click(); return true; }
    }
  ...
  return {
    id: "mistral", displayName: "Mistral", timings,
